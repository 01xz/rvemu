...
void machine_setup(Machine* m, int argc, char** argv) {
  ...
  m->state.xregs[kSp] = stack + stack_size;

  m->state.xregs[kSp] -= 8;  // auxv
  m->state.xregs[kSp] -= 8;  // envp
  m->state.xregs[kSp] -= 8;  // argv end
  ...
  for (u64 i = guest_argc; i > 0; i--) {
    ...
    m->state.xregs[kSp] -= 8;
    mmu_write(m->state.xregs[kSp], (u8*)&addr, sizeof(u64));
  }

  m->state.xregs[kSp] -= 8;  // argc
  mmu_write(m->state.xregs[kSp], (u8*)&guest_argc, sizeof(u64));
}
